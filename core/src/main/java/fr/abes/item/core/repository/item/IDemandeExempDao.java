...
public interface IDemandeExempDao extends JpaRepository<DemandeExemp, Integer> {
    @Query("select d from DemandeExemp d join d.ligneFichierExemps l where d.etatDemande.numEtat = 5 having count(l) > :limite order by d.dateModification")
    ...
    @Query("select d from DemandeExemp d join d.ligneFichierExemps l where d.etatDemande.numEtat = 5 and d.indexRecherche.code != 'DAT' having count(l) > :limite order by d.dateModification asc")
