...
public class MailerSupp extends Mailer implements IMailer {
    ...
    public void mailDebutTraitement(String mailDestinataire, Demande demande){
        ...
        String requestJson = mailToJSON(mailDestinataire, Constant.DEMANDE_SUPPRESSION_START + demande.getId() + " lancée - ILN " + demande.getIln()
    ...
    public void mailFinTraitement(String mailDestinataire, Demande demande, File f, LocalDateTime dateDebut, LocalDateTime dateFin) {
        ...
        String requestJson = mailToJSON(mailDestinataire, Constant.DEMANDE_SUPPRESSION_START + numDemande + " terminée - ILN " + demande.getIln(),
    ...
    public void mailAlertAdmin(String mailUtilisateur, Demande demande) {
        String requestJson = mailToJSON(mailAdmin, "Erreur dans Item / Suppression - ILN " + demande.getIln() + " / " + ((env.getActiveProfiles().length>0)?env.getActiveProfiles()[0]:"Local"), "Une erreur vient de se produire sur Item / Demande de suppression " + demande.getId());
        sendMail(requestJson);
